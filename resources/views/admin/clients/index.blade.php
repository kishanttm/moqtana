...
<div class="d-flex align-items-center justify-content-between mb-3">
    <div>
        <h4 class="mb-0">{{ $cmsTranslations['clients_list']->name }}</h4>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}" class="link">{{ $cmsTranslations['home']->name }}</a></li>
                <li class="breadcrumb-item active" aria-current="page">{{ $cmsTranslations['clients_list']->name }}</li>
            </ol>
        </nav>
    </div>
    <a href="{{ route('clients.create') }}" class="btn btn-lg btn-primary align-items-center d-inline-flex gap-2 fs-18">
        ...
        {{ $cmsTranslations['add_new_client']->name }}
    </a>
</div>
<div class="row">
    <div class="col-12">
        <div class="card rounded-4">
            <div class="card-body">
                <table class="Data_Table table align-middle mb-0 w-100 table-hover table-striped">
                    <thead>
                        <tr>
                            <th class="py-2">{{ $cmsTranslations['clients_name']->name }}</th>
                            <th class="py-2">{{ $cmsTranslations['type']->name }}</th>
                            <th class="py-2">{{ $cmsTranslations['mobile']->name }}</th>
                        ...
                        @forelse ($clients as $index => $client)
                            <tr>
                                @if($client->client_type === 'individual')
                                    <td>{{ $client->individual_name ?? '-'}}</td>
                                    <td>{{ $client->client_type ?? '-'}}</td>
                                    <td>{{ $client->mobile_number ?? '-'}}</td>
                                    <td>{{ $client->individual_email ?? '-'}}</td>

                                <!-- Business -->
                                @elseif($client->client_type === 'business')
                                    <td>{{ $client->company_name ?? '-'}}</td>
                                    <td>{{ $client->client_type ?? '-'}}</td>
                                    <td>{{ $client->accountant_mobile ?? '-' }}</td>
                                    <td>{{ $client->accountant_email ?? '-'}}</td>
                                @endif
                                <td>
                                    <a href="{{ route('clients.show', $client) }}" class="btn btn-sm btn-secondary align-items-center d-inline-flex" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{{ $cmsTranslations['view']->name }}">
                                    ...
                                    <a href="{{ route('clients.edit', $client) }}" class="btn btn-sm btn-secondary align-items-center d-inline-flex" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{{ $cmsTranslations['edit']->name }}">
                                    ...
                                    <a class="btn btn-sm btn-danger text-white delete-client-button" data-url="{{ route('clients.destroy', $client) }}" data-client="{{ $client }}">{{ $cmsTranslations['delete']->name }}</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
